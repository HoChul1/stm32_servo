...
int rxBufferCp;  // get pointer (read)
...
uint8_t getChar() {
	uint8_t result;
	if(rxBufferCp == rxBufferPp) return 0; //rxBufferCp == rxBufferPp -> buffer is empty
	result = rxBuffer[rxBufferCp++];
	rxBufferCp %= rxBufferMax;
	return result;
}
